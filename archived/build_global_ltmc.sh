...
echo "🌍 Building LTMC Global Binary"
echo "=============================="

# Create global LTMC launcher
cat > ltmc_global.py << 'PYEOF'
#!/usr/bin/env python3
"""
...
# Create PyInstaller spec for global binary
cat > global_ltmc.spec << 'SPECEOF'
...
a = Analysis(
    ...
    hooksconfig={},
    runtime_hooks=[],
    excludes=['tkinter', 'matplotlib', 'PIL', 'PyQt5', 'PyQt6', 'tensorflow', 'torch'],
...
echo "Building global binary..."
mkdir -p dist
pyinstaller global_ltmc.spec --distpath dist --workpath build --noconfirm --clean

if [ -f "dist/ltmc" ]; then
    chmod +x dist/ltmc
    echo "✅ Global binary created: $(pwd)/dist/ltmc"
    
    echo "Testing global binary..."
    timeout 3 ./dist/ltmc --help 2>/dev/null || echo "Binary runs (no --help implemented)"
    
    echo "🎉 Global build complete!"
    echo "Binary location: $(pwd)/dist/ltmc"
else
    echo "❌ Build failed"
    exit 1
fi

rm -f ltmc_global.py global_ltmc.spec
rm -rf build/