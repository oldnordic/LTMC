...
1. advanced_get_context_usage_statistics - Get comprehensive context usage statistics
...
def register_advanced_tools(mcp: FastMCP, settings: LTMCSettings) -> None:
    ...
    @mcp.tool()
    async def advanced_get_context_usage_statistics() -> Dict[str, Any]:
        """
    ...
    async def advanced_context_search(
        ...
        try:
            ...
            # Implement advanced semantic search with FAISS integration
            # Use FAISS service for semantic similarity search
            faiss_results = []
            if hasattr(faiss_service, 'search_vectors') and query_clean:
                try:
                    # Perform semantic vector search using FAISS
                    vector_results = await faiss_service.search_vectors(query_clean, top_k=top_k * 2)
                    
                    if vector_results:
                        # Convert FAISS results to usable format
                        for vector_id, similarity_score in vector_results:
                            faiss_results.append({
                                'vector_id': vector_id,
                                'similarity_score': similarity_score
                            })
                        logger.debug(f"FAISS returned {len(faiss_results)} semantic results")
                    
                except Exception as faiss_error:
                    logger.warning(f"FAISS search failed, falling back to text search: {faiss_error}")
                    # Continue with fallback text search below
            ...
            # Apply additional filters and merge with FAISS semantic results
            filtered_results = []
            
            # If we have FAISS semantic results, prioritize them
            if faiss_results and chat_results:
                # Create a mapping of vector_ids to chat messages for efficient lookup
                chat_by_vector_id = {}
                for msg in chat_results:
                    vector_id = msg.get('vector_id')
                    if vector_id is not None:
                        chat_by_vector_id[vector_id] = msg
                
                # Process FAISS results with semantic relevance
                for faiss_result in faiss_results:
                    vector_id = faiss_result['vector_id']
                    semantic_score = faiss_result['similarity_score']
                    
                    if vector_id in chat_by_vector_id:
                        msg = chat_by_vector_id[vector_id]
                        ...
                        # Create result with semantic relevance score
                        result = {
                            ...
                            "relevance_score": round(semantic_score, 3),
            ...
            # Fallback to text-based search if no FAISS results or insufficient results
            if len(filtered_results) < top_k:
                remaining_slots = top_k - len(filtered_results)
                used_msg_ids = {r['id'] for r in filtered_results}
                
                for msg in chat_results:
                    if msg.get('id') in used_msg_ids:
                        continue  # Skip already included messages
                    ...
                    if query_clean.lower() in content:
                        # Calculate text relevance score (simple word matching)
            ...
            return {
                ...
                "search_metadata": {
                    ...
                    "search_type": "hybrid_semantic" if faiss_results else "text_matching",
